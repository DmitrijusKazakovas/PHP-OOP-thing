<?php
//Products Class - handles all tasks related to retrieving and dispalying products
...
class Products
{
    ...
    public function get($id=NULL)
    {
        ...
        if(is_array($id))
        {
            ...
            if ($result = $this->Database->query("SELECT id, name, description, price, image FROM
            $this->db_table WHERE id IN ($items) ORDER BY name"))
            {
        ...
        else if ($id != NULL)
        {
            ...
            WHERE $this->db_table.id = ? AND $this->db_table.category_id = categories.id"))
            {
                ...
                $stmt->execute();
                $stmt->store_result();
                $stmt->bind_result($prod_id, $prod_name, $prod_decription, $prod_price, $prod_image, $cat_name);
                $stmt->fetch();

                if($stmt->num_rows > 0)
                {
                    $data = array('id' => $prod_id, 'name' => $prod_name, 'description' => $prod_decription,
    ...
    /**
     * Reutrn an array of price info for specified ids
     * @access publis
     * @param array
     * @return array
     */
    public function get_prices($ids)
    {
        ...
        //create comma seprated list of ids
        $items = '';
        foreach($ids as $id)
        {
            if ($items != '')
    ...
    /**
     * Create product table using infor from database
     * @access public
